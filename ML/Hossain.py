...
base = pd.read_excel('Dados-Hossain.xlsx', encoding = 'ISO-8859-1')
base2 = pd.read_excel('DadosTeste-Hossain.xlsx', encoding = 'ISO-8859-1')

previsores_train = base.iloc[:, 1:6].values
classe_train = base.iloc[:,6].values

previsores_test = base2.iloc[:,0:5].values
classe_test = base2.iloc[:,5].values
...
regressor.add(Dense(units = 3, activation = 'relu', input_dim = 5))
regressor.add(Dense(units = 8, activation = 'relu'))
regressor.add(Dense(units = 1, activation = 'linear'))

regressor.compile(loss = 'mean_absolute_error', optimizer = 'adam', metrics = ['mean_absolute_error'])
regressor.fit(previsores_train, classe_train, batch_size = 5, epochs = 1000)


previsoes_training = regressor.predict(previsores_train)
previsoes_test = regressor.predict(previsores_test)

from sklearn.metrics import mean_absolute_error, mean_squared_error
from math import sqrt

mae_train = mean_absolute_error(classe_train, previsoes_training)
mse_train = mean_squared_error(classe_train, previsoes_training)
rmse_train = sqrt(mean_squared_error(classe_train, previsoes_training))

mae_test = mean_absolute_error(classe_test, previsoes_test)
mse_test = mean_squared_error(classe_test, previsoes_test)
rmse_test = sqrt(mean_squared_error(classe_test, previsoes_test))
